 ...
 *
 * `priority`: passagem direta (`as TaskPriority`), SEM normalização —
 * mantém o mesmo contrato "sem tradução" já documentado no topo deste
 * arquivo pra escrita. **Gap conhecido, não fechado nesta rodada**: G49
 * (Lane B, em voo no mesmo domínio) documenta que `createProjectBaseTasks`
 * ainda grava `priority` em inglês (`"medium"`/`"high"`/`"low"`) — uma linha
 * `project_template` lida por este mapper herda esse valor sem tradução.
 * Decidido não construir um `normalizeCloudTaskPriority` aqui: G49 já está
 * em correção no lado da ESCRITA (mesmo domínio, risco de colisão/duplicação
 * de lógica — §18.3), e esta fundação escopa só os 3 itens do pacote §3
 * (mapper de leitura, passthrough de UUID, G30). Registrado para quem
 * fechar G49 decidir se a normalização de leitura também é necessária.
 */
