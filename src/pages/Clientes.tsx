...
const Clientes = () => {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Clientes"
        subtitle="Gerencie sua base de clientes, contatos e oportunidades comerciais."
        actions={
          <>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
        ...
        <KpiCard icon={UserCheck} label="Ativos" value={activeCount} tone="success" />
        <KpiCard icon={UserPlus} label="Leads / Prospects" value={leadCount} tone="primary" />
...
const ClientDetailSheet = ({
  ...
  const InfoRow = ({ label, value, icon }: { label: string; value: string; icon?: React.ReactNode }) => (
    <div className="flex items-center justify-between text-sm gap-3">
      ...
      <span className="text-foreground font-medium text-right max-w-[220px] truncate">{value || "—"}</span>
    </div>
  );
    ...
    <Sheet open={!!client} onOpenChange={(v) => !v && onClose()}>
      <SheetContent className="bg-card border-border w-full sm:max-w-[480px] overflow-y-auto">
        ...
        <div className="space-y-6 pb-6">
          {/* Commercial */}
          <Section title="Visão comercial" icon={Target}>
            <div className="orbit-card p-4 space-y-2.5">
              <InfoRow label="Tipo" value={client.serviceType} />
              <InfoRow label="Origem" value={client.origin || ""} />
              <InfoRow label="Valor potencial" value={client.potentialValue ? fmtBRL(client.potentialValue) : ""} />
              {client.totalRevenue ? (
                <InfoRow label="Receita gerada" value={fmtBRL(client.totalRevenue)} />
              ) : null}
              <InfoRow label="Próxima ação" value={client.nextAction || ""} />
              <InfoRow label="Data" value={fmtDate(client.nextActionDate)} icon={<Calendar className="h-3.5 w-3.5" />} />
            </div>
          </Section>
          ...
          <Section title="Contato" icon={Mail}>
            <div className="orbit-card p-4 space-y-2.5">
              ...
              {(client.city || client.state) && (
                <InfoRow label="Localização" value={`${client.city || ""}${client.state ? `/${client.state}` : ""}`} />
              )}
            </div>
          </Section>
                ...
                {client.tasks.map((t, i) => (
                  <div key={i} className="orbit-card p-3 flex items-center gap-3">
                    <div className={cn(
                      ...
                      t.done ? "bg-emerald-500/20 border-emerald-500/40" : "border-border",
                    )}>
                      {t.done && <CheckSquare className="h-3 w-3 text-emerald-400" />}
                    </div>
                    <span className={cn("text-sm", t.done ? "line-through text-muted-foreground" : "text-foreground")}>
                      {t.name}
                    </span>
                  </div>
                ))}
