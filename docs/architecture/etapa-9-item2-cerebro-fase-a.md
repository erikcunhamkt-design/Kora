...
Nome proposto: **`kora.ai.brain.enabled`** — segue a convenção documentada em `src/config/flags.ts` (`kora.<domain>.<feature>.<enabled|v1>`). Opt-in, default OFF — mesmo padrão de `kora.crm.supabaseWrite.enabled` e da maioria dos flags novos citados no arquivo.
...
**Nota de nome — revisão desta rodada**: a versão anterior deste doc propunha `kora.whatsapp.brain.enabled`, por analogia direta com `kora.whatsapp.campaignSender.enabled` e com o argumento de que o gate de rollout deveria seguir o ponto de consumo (hoje, só `whatsapp-bot-reply/index.ts` lê o cérebro), não o domínio do dado. Esse argumento tem um precedente real no código (`kora.quotes.supabaseCreateReceivable.enabled` é nomeado pelo ponto de origem da ação — dentro do fluxo de orçamentos — mesmo gravando em domínio de finanças), então não é descartado por ser fraco — mas pesa mais o próprio R4 desta Fase A: a tabela (`ai_brain_profiles`) já nasce desenhada como canal-agnóstica (§2.1) precisamente para servir os itens 3/4 da Etapa 9, e nomear a flag sob o domínio `whatsapp` sinaliza, incorretamente, que o cérebro é uma feature do canal WhatsApp — o tipo de sinal errado que alguém lendo `flags.ts` daqui a alguns meses, sem o contexto desta Fase A, herdaria sem questionar. `kora.ai.brain.enabled` descreve a capacidade que está sendo ligada ("composição de prompt guiada pelo cérebro"), não a superfície que a aciona primeiro. Mantém-se a intenção original de granularidade por consumidor: se um item futuro (3 ou 4) passar a consumir o mesmo perfil por outra superfície com mecânica própria, esse segundo consumo ganha sua própria flag sob o mesmo domínio `ai` (ex.: `kora.ai.knowledgeBase.enabled`) — não reabre esta.
