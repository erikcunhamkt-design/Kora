...
**G71 — Credenciais de terceiros (Gemini/Vertex do robô WhatsApp) duplicadas em `flow_data` sem redação, e sem o mesmo RLS admin-gated que o próprio repo já usa pra credencial equivalente. [ALTO → MÉDIO/ALTO após fix de código — item #2 (flow_data) FECHADO nesta rodada; itens #4/#5 (RLS) e remediação de dado já gravado em pacote do operador, `docs/qa/g71-credenciais-terceiros-pacote-operador.md`, §8-b]**
...
**G74 — 4 consumidores secundários da Ficha Técnica leem `client.technicalSheet` (campo local-only) — `mapSupabaseClientToLocalClient` nunca o popula, então qualquer ficha técnica que só existe nativamente na nuvem é invisível nos 4 lugares. [BAIXO — confirmado, não corrigido nesta rodada (doc-only), plano de fix registrado]**
...
- **Não corrigido nesta rodada — plano de fix registrado** (`etapa-5-flip-fichas-pacote.md` §11.2, rodadas F2/F3): introduzir `useBifurcatedTechnicalSheet(clientId)` (novo hook, reaproveitando `useSupabaseTechnicalSheet`/`mapSupabaseToLocalSheet`/`getTechnicalSheetDataSource` já prontos) e trocar os 4 pontos de leitura por ele. `ClientTechnicalSheetDialog.tsx` precisa de cuidado extra — é um formulário com draft editável local; só a leitura INICIAL bifurca, a gravação do diálogo continua no caminho existente hoje (local).
- **Invariante explícita pro fix futuro (reforçada pelo revisor, não descoberta nesta rodada):** o novo hook deve reusar `mapSupabaseToLocalSheet` tal como está — essa função **nunca** reconstrói `accesses`/`competitors` a partir de `raw_payload`, por desenho do G63 (perda funcional zero, confirmada). Qualquer fix deste achado que tente "completar" esse mapeamento pra também trazer `accesses` de volta reabriria exatamente a porta que o G63 fechou — fora de escopo permanentemente, não só nesta rodada.
- **Referência:** `src/hooks/useClientsDataSource.ts:7-42` (`mapSupabaseClientToLocalClient`, raiz do gap), `src/hooks/useSupabaseTechnicalSheet.ts`/`src/services/technicalSheets/supabaseTechnicalSheetToLocalMapper.ts` (única leitura cloud existente, a ser reaproveitada), G63 acima (invariante de `accesses`/`password`), G66/G58 (mesma classe "consumidor secundário não acompanhou o cutover"), `etapa-5-flip-fichas-pacote.md` §10/§11 (revalidação e plano completo).
