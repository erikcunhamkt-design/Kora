...
**G75 — "Biblioteca do cliente" (`Client.assets`) nunca teve coluna cloud; `Clientes.tsx:updateClient` monta o patch pra Supabase por uma whitelist explícita que nunca incluiu esse campo — UPDATE vazio + `toast.success` mentindo sucesso, perda de dado silenciosa. [MÉDIO — mitigação interina FECHADA nesta rodada (aviso honesto, nunca bloqueia); decisão de persistência definitiva registrada como pendência de produto]**
Achado na Fase A do domínio Materiais (`docs/qa/etapa-5-flip-materiais-pacote.md`), confirmado com "vai" do revisor. Campo sem coluna + UPDATE de lista explícita = perda silenciosa com sucesso falso; **a classe G67/G68 tem uma variante pior: quando o destino nem existe.** G67/G68 são "campo omitido do mapper de leitura" ou "cast quebrando com uuid" — aqui não há sequer uma coluna cloud pra receber o valor; o patch construído fica `{}` e a chamada resolve normalmente, então nem o `catch` de erro pega o problema.

- **Mecanismo:** `ClientLibrarySection.tsx` (único produtor, "Biblioteca do cliente" na aba "Materiais" do perfil) chama `onChange` → `ClientProfileDrawer.tsx:1112` (`onUpdateAssets`) → `Clientes.tsx:828` (`updateClient(id, { assets })`). Em modo Supabase (default de qualquer sessão com workspace ativo, sem toggle manual nesta tela), `updateClient` monta um patch campo a campo de uma whitelist que nunca inclui `assets` — o patch fica vazio, `supabaseUpdate` (quando chamado) não tem nada de fato pra persistir, e o `toast.success("Cliente atualizado no Supabase.")` genérico dispara mesmo assim. A UI parece certa por causa de uma atualização otimista só em memória (`setSelectedClient`) — some no próximo refetch.
- **Mitigação interina aplicada (v1, aprovada como proposta no pacote):**
  1. `Clientes.tsx:updateClient` — quando o patch fica vazio, pula a chamada de rede (`if (Object.keys(patch).length > 0)`); quando a mudança é especificamente de `assets`, troca o `toast.success` genérico por `toast.warning("Material salvo só neste dispositivo — a Biblioteca do cliente ainda não sincroniza com a nuvem.")`. Nunca bloqueia — mesmo padrão da ressalva (a) de Financeiro (avisar, não impedir).
  2. `clientAssetsStorage.ts` (`validateMaterialFile`) — achado colateral do pacote (R3): a validação client-side aceitava PDF/DOCX/XLSX/TXT até 8MB, mas o bucket `client-assets` (`supabase/migrations/20260530030000_create_client_assets_storage.sql`) só permite `png/jpeg/webp` até 2MB — qualquer material fora disso passava na tela e falhava no upload real com o erro genérico do Supabase. Alinhado à policy real; subir a policy (novos MIME types/tamanho) fica registrado em comentário como decisão do operador (migration nova), não algo pra revalidar sozinho de novo.
- **Testes** (`Clientes.test.tsx`, novo arquivo — primeira cobertura desta página; `clientAssetsStorage.test.ts`, novo arquivo): adicionar material em modo Supabase dispara o `toast.warning` esperado, nunca bloqueia — falha contra o código anterior; regressão — modo local continua com o `toast.success` normal; PDF/DOCX/XLSX/TXT e imagem 2-8MB rejeitados pela validação — falham contra o código anterior; PNG/JPEG/WebP até 2MB aceitos — regressão. Prova fail→fix→pass por patch (G65, sem `git stash`): 3 testes falham contra o código antigo (1 em `Clientes.test.tsx`, 2 em `clientAssetsStorage.test.ts`), 5/5 verdes após reaplicar.
- **NÃO resolvido nesta rodada, decisão de produto pendente (3 opções, `etapa-5-flip-materiais-pacote.md` §10):** (a) nova coluna JSONB `assets` em `public.clients`; (b) unificar com `technicalSheet.assets` (já migrado, mesma estrutura) — resolveria também o R2 do pacote (confusão de nomenclatura entre os 2 campos-irmãos); (c) aceitar local-only permanentemente por decisão de produto — já é o estado de fato após esta mitigação, só falta ser uma decisão explícita em vez de um gap não-decidido.
- **Referência:** `docs/qa/etapa-5-flip-materiais-pacote.md` (pacote completo, achado crítico + R1-R4), G67/G68 acima (mesma classe, variante sem coluna cloud), `Financeiro.tsx` (ressalva (a) do sign-off — mesmo padrão de aviso honesto reaproveitado aqui).
...
**G77 — Gate de segurança do G76 (`canCompleteTask`/guarda de `completeTask`), correto quando escrito, vira gate FÓSSIL bloqueando a MAIORIA por default assim que a Fase C do flip de Tarefas entra — mesma classe do G33/G55 ("gate cobrindo a ação errada depois que o cenário-base mudou"). [MÉDIO — confirmado e FECHADO por `fb4d508` (LANE B — completeTask nativo em 3 caminhos), antes do flip entrar em `main`]**

Achado durante a varredura de gates fósseis desta rodada (Fase C do flip de Tarefas, item 3 da instrução — grep exaustivo por `getTasksDataSource`/`useSupabaseTasksWriteFlag` fora do domínio de tasks).

- **Mecanismo:** `canCompleteTask = getTasksDataSource() !== "supabase"` (`DayCenter.tsx:142`, duplicado em `useDayCenterActions.ts:55`) — o guard do G76, escrito e correto quando `dataSource` nascia default `"local"` (opt-in raro pra `"supabase"`). Com o flip desta rodada, `getTasksDataSource()` passa a devolver `"supabase"` por default pra QUALQUER sessão que nunca tocou o seletor — ou seja, a partir do momento em que este flip entrar em `main`, o botão "Concluir" da Central do Dia (`QuickAction`, `DayCenter.tsx:478`, `if (!canCompleteTask) return null;`) **some por default pra maioria dos usuários**, não só pro caso raro que o G76 mirava. O toast de defesa-em-profundidade (`useDayCenterActions.ts:56`, "ainda não funciona em modo Supabase") continua tecnicamente correto (a implementação de `completeTask` de fato ainda chama só o mutator LOCAL de `useTasks()`, nunca o `moveTask` nativo que a B5 já construiu) — não é um G29 (texto não ficou desatualizado/errado), é uma mudança de POPULAÇÃO afetada (de exceção rara pra maioria).
- **Por que não é um bug do G76**: o guard do G76 está fazendo exatamente o que foi desenhado pra fazer — nunca chamar `Number(uuid)` sem uma guarda. O problema é que a suposição implícita por trás dele ("modo Supabase é raro, poucos usuários acionam este caminho") deixa de valer no exato momento em que a Fase C flipa o default do MESMO domínio que o guard consulta.
- **NÃO corrigido nesta rodada** — por 2 razões: (1) o escopo autorizado desta rodada é "flipar os 2 defaults", não construir uma feature nova; (2) o fix correto de verdade — trocar a chamada de `completeTask` pro `moveTask` nativo de `useSupabaseTasksAll` quando `dataSource==="supabase" && writeEnabled` (mesmo padrão já usado em `Tarefas.tsx`, B5) — é trabalho real de wiring num arquivo historicamente de outra lane (`DayCenter.tsx`/`useDayCenterActions.ts`, G76), fora do "flip de default" mecânico pedido aqui. Simplesmente relaxar o guard sem trocar a chamada reintroduziria o próprio bug que o G76 fechou (`Number(uuid)=NaN`, no-op silencioso com toast de sucesso mentindo) — pior que deixar como está.
- **Consequência prática pro operador** (registrada quando o achado foi aberto, antes do fix): a partir do flip, o botão "Concluir tarefa" na Central do Dia desapareceria por default pra quem nunca mexeu no seletor de fonte de Tarefas — feature que funcionava (localmente) sumindo sem aviso visível. Recomendação registrada então: rodada de acompanhamento pra wire `completeTask`/`useDayCenterActions` no `moveTask` nativo (B5 já pronto pra isso), prioridade ALTA.
- **FECHADO por `fb4d508` (Lane B, 23/ago/2026), ANTES deste flip aterrissar em `main`** — exatamente a recomendação acima: `completeTask` (`DayCenter.tsx`/`useDayCenterActions.ts`) ganhou 3 caminhos — (a) `dataSource=local`: mutator local de sempre; (b) `dataSource=supabase` + `useSupabaseTasksWriteFlag` ligada: roteia pro `moveTask` NATIVO de `useSupabaseTasksAll()` (`taskId` como string, sem `Number()`), mesmo molde de `cloudWriteMode` já usado em `Tarefas.tsx` (B5); (c) `dataSource=supabase` + flag desligada: guarda do G76 continua valendo (único caso que resta sem mutator cloud-aware). `canCompleteTask` virou `getTasksDataSource() !== "supabase" || tasksWriteEnabled` — com os 2 defaults desta Fase C (`dataSource` e a flag de escrita, ambos "supabase"/ligado por padrão), a combinação DEFAULT pós-flip cai no caminho (b), não no (c): o botão "Concluir" continua funcionando por default pra quem nunca tocou nenhum dos 2 seletores — o gate fóssil deixa de ser fóssil antes mesmo do flip valer. O toast "ainda não funciona em modo Supabase" só dispara agora no caso (c) (dataSource=supabase E flag de escrita desligada — 2 desvios explícitos do default, não mais a resposta padrão de qualquer sessão em modo nuvem).
- **Referência:** G76 acima (guard original, correto na origem), G33/G55 (mesma classe "gate cobrindo ação errada após mudança de cenário-base"), B5 (`kora-hub-auditoria-e-plano.md`, `useSupabaseTasksAll.moveTask` — o mutator nativo que faltava ligar, agora ligado).
...
- **Mecanismo:** o comentário (duplicado nas linhas 88-94 e 95-101, cópia-e-cola do mesmo texto) diz literalmente "a escrita nativa em modo Supabase pra Tarefas é a B5 do plano, ainda não existe — mover/criar uma tarefa lida da nuvem por aqui é um no-op no array local". Isso era verdade quando escrito (rodada B4, 22/ago/2026), mas B5 (`5e1829d`, `1dea136`) já fechou em `Tarefas.tsx` — a escrita nativa EXISTE no repo, só não está fiada a `ProjectDetailDrawer.tsx` (`addTask`/`moveTask` aqui continuam vindo de `useTasks()` local, sem `cloudWriteMode`). O comentário não descreve mais um fato do domínio ("B5 não existe") — descreve, sem dizer, uma decisão de escopo não tomada explicitamente ("este arquivo não recebeu o wiring de B5").
...
- **Mecanismo 1 (projetos):** o formulário de criação de projeto captura o cliente como texto livre (`clientName`), mas o mapper de escrita só persiste `client_id` (uuid, via resolução de FK) — esse campo nunca é preenchido a partir do texto digitado, porque não há resolução nome→uuid no formulário. O projeto é criado com sucesso (toast de sucesso, sem erro); ao reler da nuvem, `clientName` volta vazio — o vínculo nunca existiu de fato, sem a UI ter avisado em nenhum momento.
- **Mecanismo 2 (tarefas), mesma classe:** `Tarefas.tsx:86` — `const clientsList = ["Acme Corp", "Studio Zen", "Nova Design", "FitTrack", "Café & Arte", "Brand Co", "StartUp X"];` é uma lista hardcoded, usada nos seletores de cliente das linhas 580 e 1212. Não oferece nenhum cliente real do workspace — mesmo com clientes reais cadastrados e em modo Supabase, o seletor de "cliente" de uma tarefa só mostra os 7 nomes fictícios do mock.
- **Consequência combinada:** nenhuma feature que dependa de vínculo cliente↔tarefa/projeto funciona hoje em modo Supabase — inclusive `ClientActivitiesTab`/"Histórico de Relacionamento" mostrando eventos de tarefa de um cliente (Caso 7.4 do runbook de Tarefas, fechou parcial por causa direta deste achado).
