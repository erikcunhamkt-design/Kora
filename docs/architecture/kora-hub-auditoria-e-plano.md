...
**G31 — Baseline de lint: `npm run lint` passa de 0 erros/29 warnings pra 0 erros/3 warnings. Os 3 remanescentes NÃO são "sobra a corrigir depois" — cada um tem motivo registrado pra não virar zero. [BAIXO — baseline novo, registrado pra nenhuma lane futura "queimar" o #3 por engano]**
Achado durante a rodada `qualidade-lint-warnings-zero` — G30 fica reservado à Lane A (fix em voo no momento desta rodada, não colidir). Dos 29 warnings originais (todos `react-refresh/only-export-components` ou `react-hooks/exhaustive-deps`), 26 eram mecânicos (arquivo misturando export de componente com export de valor não-componente → extraído pra arquivo irmão; dependência de hook genuinamente morta ou de referência estável → ajustada) e foram corrigidos sem `eslint-disable` e sem mudança de comportamento em runtime — ver commit `0211532`/`a01d56c` pro detalhamento arquivo-a-arquivo. **Os 3 abaixo ficam de pé — não são preguiça, são decisão registrada:**

- **`WhatsAppBotConfig.tsx:178`** (`useCallback` de `loadSettings` sem `nodes` no dep array) — `loadSettings` está no dep array de um `useEffect` (linha 183). Adicionar `nodes` faria esse efeito re-disparar (novo fetch ao Supabase) a cada edição do fluxo visual, não só na montagem — mudança de comportamento real, não um ajuste de 1 linha. **Correção correta, se um dia for feita:** `useRef` espelhando `nodes` pra ler o valor atual sem entrar no dep array (padrão "latest ref"). Fica registrado como pacote futuro, não uma pendência urgente — o bug de staleness que o warning aponta só afeta o caminho de fallback legado (sem `flow_data` salvo no banco), superfície estreita.
...
- **Por que registrar isso agora:** os 3 sobreviveram por *decisão*, não por *lacuna* — sem este registro, o próximo "queima os warnings restantes" trataria os 3 como pendência de faxina e corrigiria pelo manual do eslint, reintroduzindo o problema original que cada um evita (fetch em loop, formulário sujo, teste cego a regressão).
