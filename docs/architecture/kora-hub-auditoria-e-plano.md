...
**G34 — `mirrorProjectToSupabase` espelhava um projeto empobrecido: `source`/`quote_id`/`deliverables` perdidos sempre que o projeto vem de uma quote NATIVA DA NUVEM. [MÉDIO — confirmado e FECHADO, classe "espelho com payload incompleto — perda de fidelidade local→nuvem"]**
