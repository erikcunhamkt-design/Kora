...
- **Limitação conhecida, encontrada pelo revisor na revisão do fix (não corrigida nesta rodada, de propósito) — a alegação inicial de "idempotência do import" estava ERRADA:** o `lead.wonAt ||` dentro do ramo "fechado" só preserva um valor pré-existente SE `lead.wonAt` já vier preenchido no objeto `Lead` recebido pela função — cenário testado (`crmOpportunityMapper.test.ts`) mas que **não reflete o fluxo real**: `Lead.wonAt` nunca é escrito por nenhum caminho local (mesmo grep exaustivo do achado original), então no uso real de `useLocalOpportunitiesImport.ts:180` (`raw: local`, vindo de `useLeads()`) esse campo é sempre `undefined`. **Prova (teste novo, fail→pass via fake timers):** reimportar o MESMO lead "fechado" 2x gera um `won_at` NOVO a cada vez — a 2ª importação não preserva o timestamp da 1ª. Não há guarda no ponto de upsert (`crmOpportunitiesRepository.upsertImportedOpportunity:146-162`, `.upsert()` cru, sem `SELECT` prévio da linha existente por `source_local_id`) que evite isso.
  ...
  - **Por que não foi corrigido agora (opção (a) descartada por invasiva, opção (b) aplicada):** corrigir de verdade exigiria buscar a linha existente por `source_local_id` ANTES do upsert (`crmOpportunitiesRepository.ts`/`useLocalOpportunitiesImport.ts`) pra decidir se o `stage` mudou e só então recalcular `won_at` — round-trip de leitura extra por item importado, reestrutura o ponto de escrita, risco de corrida entre o `SELECT` e o `UPSERT`. Avaliado como invasivo demais pra esta rodada de fix; mantido o fix atual (que já resolve o achado original — limpeza em transições pra fora de "fechado") + esta limitação documentada, em vez de alegar uma garantia que não existe.
  - **Backlog:** se o operador considerar a janela de exposição relevante, a correção completa é: em `useLocalOpportunitiesImport.ts` (ou dentro de `upsertImportedOpportunity`), buscar a oportunidade existente por `source_local_id` antes do upsert e, se o `stage` da busca bater com o `stage` novo, usar o `won_at`/`lost_at` já gravados em vez de recalcular. Não dimensionado nem agendado nesta rodada.
- **Testes:** `crmOpportunityMapper.test.ts`, describe "G57" — 5 casos: `stage="perdido"` com `wonAt` stale → `null` (falha contra o código anterior, confirmado por `git stash` nas duas direções); as 4 variações de stage não-fechado → sempre `null`; `stage="fechado"` sem `wonAt` prévio → seta novo; `stage="fechado"` COM `wonAt` já no objeto recebido → preserva (cenário hipotético, rotulado como tal no teste); e o teste novo desta rodada — reimportar o MESMO lead 2x (fake timers, `wonAt` sempre `undefined` como no fluxo real) → `won_at` muda entre a 1ª e a 2ª chamada, prova da ausência de idempotência real.
