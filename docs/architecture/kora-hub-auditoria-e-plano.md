...
- **Consequência prática pro operador**: a partir deste flip, o botão "Concluir tarefa" na Central do Dia desaparece por default pra quem nunca mexeu no seletor de fonte de Tarefas — feature que funcionava (localmente) some sem aviso visível (o toast de defesa nunca dispara, porque o botão já não está lá pra clicar). Recomendação: rodada de acompanhamento pra wire `completeTask`/`useDayCenterActions` no `moveTask` nativo (B5 já pronto pra isso) — prioridade sugerida ALTA, dado que afeta a maioria assim que o flip for pra produção.
- **Referência:** G76 acima (guard original, correto na origem), G33/G55 (mesma classe "gate cobrindo ação errada após mudança de cenário-base"), B5 (`kora-hub-auditoria-e-plano.md`, `useSupabaseTasksAll.moveTask` — o mutator nativo que falta ligar aqui).
