...
Herdando as lições registradas em G29 (banner de "modo leitura" sobrevivendo ao flip real da escrita — texto de UI desatualizado depois que CRUD virou funcional) e a disciplina operacional de "criar ANTES de consultar" (evitar SELECT prematuro contra estado que ainda não existe na base nova):
