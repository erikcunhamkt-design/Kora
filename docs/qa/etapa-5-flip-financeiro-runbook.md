...
| `HOMOLOG-FIN-quote` | Quote sintética aprovada — origem do caso 4.2 (CRM) e do caso 4.3-b (G56 — repetição de propósito contra um `quote_id` que já tem recebível vivo, provando a colisão) |
| `HOMOLOG-FIN-quote-2` | 2ª quote sintética aprovada, própria (G56) — origem do caso 4.3 (Vendas), pra não colidir com o recebível de `HOMOLOG-FIN-quote` criado no 4.2 |
| `HOMOLOG-FIN-transacao-A` | Transação criada nativa, direto na tela principal em modo Supabase — casos 1, 2, 3, 5 |
| `HOMOLOG-FIN-transacao-B` | Recebível gerado via `CreateReceivableDialog` (CRM) a partir de `HOMOLOG-FIN-quote` — caso 4.2 |
| `HOMOLOG-FIN-transacao-C` | Recebível gerado via `QuoteToReceivableDialog` (Vendas) a partir de `HOMOLOG-FIN-quote-2` (G56 — quote PRÓPRIA, não a mesma do 4.2) — caso 4.3 |
...
- **Lição nova (Financeiro, G56 — não herdada de `projects`, registrar daqui pra frente):** quando 2 caminhos de escrita diferentes podem alvejar a MESMA constraint de idempotência (aqui, os 2 diálogos de recebível vs. `ux_ft_receivable_from_quote`, 1-por-quote), um caso de homologação que exercita os 2 caminhos precisa usar entidades sintéticas DISTINTAS pra cada um (ver `HOMOLOG-FIN-quote`/`HOMOLOG-FIN-quote-2`, §3.1) — a menos que o objetivo explícito do passo seja testar a colisão de propósito (como o 4.3-b). Reusar a mesma entidade "porque já existe" sem verificar a constraint por baixo é como o Caso 4.3 original acabou testando um cenário sem querer (colisão) achando que testava outro (caminho feliz).
...
**Emenda pós-G56 (achado ao vivo, Caso 4.3 original)** — `ux_ft_receivable_from_quote` (`20260704120000_etapa3_unique_receivable_from_quote.sql:44-46`) é um índice único parcial em `(quote_id) WHERE source='quote' AND type='receivable'`: no máximo 1 recebível VIVO por orçamento, **independente de qual dos 2 diálogos cria**. O Caso 4.3 original reusava a MESMA `HOMOLOG-FIN-quote` do 4.2 — o 2º dialog (Vendas) colidia com a constraint, e `createReceivableFromQuote` recuperava do 23505 devolvendo a linha do 4.2 (`-B`) em vez de criar a `-C` esperada: `category`/`payment_method` da tela de Vendas nunca chegavam na nuvem, sem aviso nenhum (fix G56, `kora-hub-auditoria-e-plano.md`). O 4.3 abaixo foi corrigido pra usar uma quote PRÓPRIA (`HOMOLOG-FIN-quote-2`), testando o caminho FELIZ do mirror de Vendas sem colidir de propósito; o novo 4.3-b testa a colisão de propósito, contra `HOMOLOG-FIN-quote` (que o 4.2 já usou).

| Passo | Ação | Esperado | Prova |
|---|---|---|---|
| 4.1 | Setup | Criar `HOMOLOG-FIN-quote` E `HOMOLOG-FIN-quote-2` (ambas aprovadas), vinculadas a um cliente sintético — 2 quotes distintas, G56: ver "Emenda pós-G56" acima | As 2 quotes existem (local ou nuvem, conforme o modo do domínio `quotes` no momento) | Visual |
...
| 4.3 | **`QuoteToReceivableDialog.tsx`** (Vendas) — gerar `HOMOLOG-FIN-transacao-C` a partir de `HOMOLOG-FIN-quote-2` (G56 — quote PRÓPRIA, sem recebível prévio), escolhendo categoria/forma de pagamento na tela | Grava local (`:92-109`) **e** dispara o espelho (`mirrorReceivableToSupabase`, `:144-166`) — mesmo aviso de best-effort do 4.2, sem colisão (quote própria) | Visual (local imediato) + `SELECT quote_id, source, category, payment_method FROM public.financial_transactions WHERE title = 'HOMOLOG-FIN-transacao-C';` → `quote_id` preenchido (o de `HOMOLOG-FIN-quote-2`), `source = 'quote'`, **`category`/`payment_method` preenchidos com o que foi escolhido na tela** (diferente do 4.2 — ver tabela acima) |
| 4.3-b (G56) | **`QuoteToReceivableDialog.tsx`** (Vendas) — repetir a geração, agora a partir de `HOMOLOG-FIN-quote` (a MESMA do 4.2, que já tem `HOMOLOG-FIN-transacao-B` vivo na nuvem) — de propósito, provando a colisão | Local grava normalmente (transação local nova é criada, correta) — mirror colide com `ux_ft_receivable_from_quote`, `createReceivableFromQuote` recupera do 23505 devolvendo a linha de `HOMOLOG-FIN-transacao-B` (`title` diferente do enviado) — **`toast.warning` explícito de colisão** ("Este orçamento já tem uma conta a receber na nuvem...", fix G56), NÃO o toast silencioso de sucesso puro | Visual (toast de colisão aparece) + `SELECT count(*) FROM public.financial_transactions WHERE quote_id = (SELECT id FROM quotes WHERE title = 'HOMOLOG-FIN-quote') AND source = 'quote' AND type = 'receivable' AND deleted_at IS NULL;` → **1** (continua só a `-B`, nenhuma linha nova criada pela repetição) |

**Emenda pós-retest (Caso 4.3/4.3-b, LANE A) — fluxo real de abertura do diálogo de Vendas tem 3 gatilhos, não 1**: os passos 4.3/4.3-b acima descrevem "gerar `HOMOLOG-FIN-transacao-C`" como se fosse sempre um clique explícito em "Gerar conta a receber" — mas isso é só 2 dos 3 gatilhos reais que abrem `QuoteToReceivableDialog.tsx` (`QuotesSection.tsx`): (a) menu ⋯ → "Gerar conta a receber" (`:606-608`); (b) painel de preview → atalho "Gerar conta a receber" (`:1267-1273`); **(c) painel de preview → botão "Aprovar" (`onApprove`, `:677-686`) abre o MESMO diálogo sozinho, 250ms depois de aprovar, sem clique nenhum em "Gerar conta a receber"**. Quem executar o Caso 4.3 aprovando `HOMOLOG-FIN-quote-2` PELO PREVIEW (em vez do menu ⋯) vai ver o diálogo aparecer automaticamente — comportamento esperado, não um bug de teste, mas precisa ser reconhecido pra não confundir "o diálogo abriu sozinho" com um vermelho. Detalhe completo e achado colateral (`financeEntryId` nunca atualiza em modo Supabase, então o atalho/menu continuam oferecendo "Gerar conta a receber" mesmo depois de já ter um) em `kora-hub-auditoria-e-plano.md`, adendo do G56.
...
**Ressalvas registradas (não bloqueiam o HOMOLOGADO, não viram vermelho) — (a) e (b) resolvidas pós-homologação, LANE A:**
- **(a) — RESOLVIDA (`67a9a2d`).** O aviso de UX previsto pro uso dos 4 campos sem coluna cloud (`recurrence`/`supplierId`/`cashAccountId`/`notes`, §5.1) não disparava na prática — só "observações" (`notes`) faltava na checagem dos 2 diálogos de criação (recorrência/fornecedor já avisavam); `cashAccountId` não tem controle de formulário em nenhum dos 2, nada a avisar. Fix: "observações" adicionado ao array de gaps existente. Detalhe completo em `kora-hub-auditoria-e-plano.md`, backlog de UX.
- **(b) — RESOLVIDA v1 (branch `etapa-5-financeiro-edit-dialog`, esta rodada).** `EditTransactionDialog` novo na lista Supabase — edita título/descrição/valor/vencimento/status/categoria/forma de pagamento (campos com coluna real), locais-only omitidos, FKs fora da v1 (registrado como v2). Detalhe completo em `kora-hub-auditoria-e-plano.md`, backlog de UX.
