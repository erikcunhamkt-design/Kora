...
- **Estado herdado, incorporado sem rediscussão:**
  ...
  - **R2 (`tarefas-r2-auditoria.md`)**: contenção **(a) ADOTADA** pelo revisor em 14/ago/2026 — `supabaseOperationalDashboard`/`projectsSupabaseCreateBaseTasks`/`tasksSupabaseStatusTransition` seguem OFF por política até a decisão de flip. Contenção (b) (aviso na UI do painel experimental) condicionada ao volume medido pelas 8 SQLs do §1 daquele doc — **ainda não rodadas** (Code não acessa banco, protocolo §0/§6). Este pacote formaliza essa quantificação como pré-requisito de fase (§2).
...
## 2. Reconciliação R2 — pré-requisito formal de fase, não um "nice to have"

**Diferença estrutural em relação a Financeiro**: lá, nenhuma linha de `financial_transactions` foi gravada por um caminho fora do vocabulário local — o risco era hipotético. Aqui, `docs/qa/tarefas-r2-auditoria.md` já mapeou **dois caminhos de escrita cloud-nativos ativos hoje** (`createProjectBaseTasks`, `updateTaskStatus`), alcançáveis via o painel experimental mesmo com as flags em OFF por padrão — ou seja, **é possível que dado divergente já exista em produção**, não é uma hipótese de desenho futuro. É por isso que o brief classifica isso como "o problema singular deste domínio": nenhum dos outros dois flips do G1 (Projetos, Financeiro) teve essa característica.

### 2.1 As 8 SQLs viram passo formal do pré-flip

`tarefas-r2-auditoria.md` §1 já tem as 8 queries prontas (somente `SELECT`, sem gate de credencial do protocolo — não é sessão de DDL). Este pacote formaliza que **a Fase B não desenha a migração de dado de reconciliação sem os números dessas 8 queries em mãos primeiro**:

| # | Query | O que decide |
|---|---|---|
| 1.1 | Total por `source`, ativas vs. soft-deletadas | Tamanho absoluto do problema — se zero, boa parte do §5 (convivência) vira não-evento |
| 1.2 | `source_local_id IS NULL` por `source` | Quantifica exatamente o "buraco" que nenhum merge automático resolve sozinho |
| 1.3 | Por `workspace_id` | App é multi-tenant — decide se a estratégia do §5 pode ser uniforme ou precisa ser por-workspace |
| 1.4 | Vocabulário de `status` em uso | Confirma (ou refuta) a suposição de "zero linhas fora do vocabulário" antes de qualquer CHECK (§1.1/G40) — **crítico**, porque diferente de Financeiro, já sabemos que existe pelo menos 1 produtor (`createProjectBaseTasks`) gravando `status='todo'` em inglês (ver §3.1) |
| 1.5 | Idade do dado por `source` | Calibra urgência — buraco antigo vs. recente |
| 1.6 | Tarefas-base por projeto | Volume de `project_template` especificamente — insumo direto do §5 |
| 1.7 | Volume já importado (`source_local_id IS NOT NULL`) | Contraponto de escala — não é risco, é o que já reconcilia |
| 1.8 | Proxy de drift pós-criação (`updated_at > created_at`) | Sinaliza candidatas a checar manualmente se `updateTaskStatus` já foi exercitado contra alguma linha com par local |

**Ordem obrigatória**: rodar 1.1→1.8 (operador, fora do Code) **antes** de qualquer desenho de migração de dado ganhar detalhe além do que já está no §5 deste pacote. Se os números vierem zero ou desprezíveis (cenário mais provável, dado que as 2 flags nativas são default OFF e o painel é rotulado "Experimental"), o §5 colapsa pra decisão (a) trivialmente — se vierem altos, o §5 precisa da decisão (c) com backfill.

### 2.2 Caminhos que AMPLIAM a divergência — congelados por contenção (a), não corrigidos

Reafirmando `tarefas-r2-auditoria.md` §2: `createProjectBaseTasks` (linhas novas sem `source_local_id`, nunca reconciliáveis pelo import geral) e `updateTaskStatus` (muda linha existente sem tocar o local correspondente) seguem sendo os 2 caminhos que ampliam o buraco — nenhum dos dois foi alterado nesta rodada nem é proposto pra alteração aqui (fora de escopo doc-only). A contenção (a) — as 3 flags seguirem OFF por política — é a única mitigação ativa até a Fase B decidir o §5.
...
### Opção (c) — Reconciliar uma vez, depois fundir (recomendação desta rodada)
...
### 6.1 Sequência (mesma ordem de Financeiro/Projetos, com 1 passo novo antes de tudo)

1. **Pré-requisito (novo, não existe nos outros 2 flips do G1)**: operador roda as 8 SQLs do §2.1, decisão do §5 fecha com números reais em mãos.
2. **Fase B (código)**: migration do §1.1 (4 campos genuinamente preventivos) + fix de vocabulário do `createProjectBaseTasks` (§3.1) + migration de `status`/`priority` CHECK só depois disso + passthrough de UUID (§3.2) + `mapSupabaseTaskToLocal`/`listTasks` (§3.4) + `useBifurcatedTasks` e os 8 consumidores (§4.1) + tratamento de `ClientActivitiesTab.tsx` com atenção de coordenação (§4.2) + espelho G22 em `QuoteToProjectDialog.tsx` + G30 aplicado a qualquer escrita nova (§3.5) + a opção escolhida no §5, incluindo backfill se necessário. `tsc`/lint/testes verdes, PARA pra aprovação.
3. **Fase C (flip dos defaults)**: `kora.tasks.dataSource.v1` → `supabase` (flag nasce nesta mesma fatia — não existe hoje, Fase A §1.3).
4. **Fase D (homologação B.3)**: runbook próprio, cenário sintético (`HOMOLOG-FLIP-tarefas`).
...
5. **Tarefas-base coexistindo com tarefas locais migradas**: caso central do §5 — comportamento esperado depende da opção escolhida, definir ANTES do teste, não durante.
...
### 6.4 Fechamento — estimativa comparativa final

Confirma e detalha a conclusão qualitativa já registrada na Fase A (`etapa-5-flip-tarefas-fase-a.md` §6): **Tarefas é o maior dos três flips do G1**, agora com o desenho de Fase B completo em vez de comparação por risco. Itens que Financeiro não teve:

1. **Pré-requisito de quantificação (§2)** — as 8 SQLs não são opcionais, são gate de fase. Financeiro nunca precisou disso (nenhum dado divergente pré-existente).
2. **Decisão de convivência com dado nativo já existente (§5)** — 3 opções com trade-offs reais, nenhuma trivial. Financeiro não teve equivalente (os 2 produtores de `financial_transactions` sempre gravaram no formato certo, nunca houve um "3º dialeto" nativo tipo `project_template`).
3. **Fix de vocabulário do 2º produtor antes do CHECK (§3.1)** — Financeiro conseguiu o CHECK preventivo de graça; aqui precisa de 1 fix primeiro, mesmo padrão do O12/G40 original, não do desenho limpo que Financeiro teve.
4. **8 consumidores a bifurcar (§4.1)**, vs. 5 de Financeiro — 3 a mais (`useTaskReminders.ts`, `useDayCenterActions.ts` como hook separado do `DayCenter.tsx`, e `QuoteToProjectDialog.tsx` como produtor em vez de só leitor).
5. **`ClientActivitiesTab.tsx` na 3ª rodada de edição** (depois de Projetos e, presumivelmente, Financeiro) — mesmo arquivo, mais peso acumulado, com uma proposta de refactor preventivo explicitamente colocada pra decisão (§4.2).
...
- `docs/qa/tarefas-r2-auditoria.md` — quantificação SQL (§2.1), contenção (a) adotada 14/ago/2026
